...
terraform {
  ...
  backend "s3" {
    bucket = "terraform-backend-pushups"
    key    = "state"
    region = "us-east-1"
  }
...
data "aws_iam_policy_document" "lambda_execution_role" {
  ...
  statement {
    ...
    resources = ["${aws_dynamodb_table.storage-table.arn}"]
  }
}
...
resource "aws_dynamodb_table" "storage-table" {
  name           = "${local.project_name}-store"
...
resource "aws_lambda_function" "lambda" {
  ...
  handler       = "pushups.app.lambda_handler"
  runtime       = "python3.12"
  architectures = ["arm64"]
  environment {
    variables = {
